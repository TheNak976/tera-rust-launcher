...
const App = {
        ...
        try {
            ...
            if (this.state.isAuthenticated && this.Router.currentRoute === 'home') {
                if (!UPDATE_CHECK_ENABLED) {
                    console.log('Updates are disabled, skipping update check and server connection');
                    this.setState({
                        isUpdateAvailable: false,
    ...
    updateUIElements() {
        ...
        if (!UPDATE_CHECK_ENABLED) {
            if (elements.dlStatusString) elements.dlStatusString.textContent = this.t('NO_UPDATE_REQUIRED');
            if (elements.progressPercentage) elements.progressPercentage.textContent = '(100%)';
            if (elements.progressPercentageDiv) elements.progressPercentageDiv.style.width = '100%';
            ...
            if (elements.downloadedSize && elements.downloadedSize.parentElement) elements.downloadedSize.parentElement.style.display = 'none';
            if (elements.totalSize && elements.totalSize.parentElement) elements.totalSize.parentElement.style.display = 'none';
    ...
    async handleLaunchGame() {
        ...
        if (UPDATE_CHECK_ENABLED && this.state.isUpdateAvailable) {
            console.log('Updates are available, please update before launching the game');
